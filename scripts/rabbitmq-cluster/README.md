...
# 管理界面访问 (需要临时映射端口)
# 映射rabbitmq1的管理界面
docker-compose exec rabbitmq1 rabbitmqctl eval 'application:ensure_all_started(rabbitmq_management).'
docker run --rm -p 15672:15672 --network rabbitmq-net --name temp-mgmt \
  -v $(pwd)/haproxy/haproxy.cfg:/tmp/haproxy.cfg haproxy:2.8 \
  haproxy -f /tmp/haproxy.cfg
# 然后访问: http://localhost:15672

# 或直接映射单个节点端口进行管理
docker-compose up -d
docker run -d --name rabbitmq-mgmt --network rabbitmq-net -p 8080:15672 \
  rabbitmq:3.12-management \
  bash -c "rabbitmqctl wait /var/lib/rabbitmq/mnesia/rabbit@rabbitmq1.pid && \
...
- **5672**: AMQP协议端口 (通过HAProxy)
- **15672**: 管理界面端口 (各节点需要单独映射)
- **25672**: 集群通信端口 (内部使用)
