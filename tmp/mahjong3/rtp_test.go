...
func TestRtp(t *testing.T) {
	...
	for {
		...
		// 更新 isFreeRound 状态
		betService.isFreeRound = betService.scene.Stage == _spinTypeFree || betService.scene.Stage == _spinTypeFreeEli
		...
		if betService.isFreeRound {
			...
			if betService.scene.RoundMultiplier > 0 && betService.isRoundOver {
				baseWinTime++
			}
		}
