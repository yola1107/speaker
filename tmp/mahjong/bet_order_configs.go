...
func (s *betOrderService) initSpinSymbol(stage int8) [_colCount]SymbolRoller {

	switch stage {
	case _spinTypeBase:
		return s.getSceneSymbol(s.gameConfig.RollCfg.Base, false, stage)
	case _spinTypeFree:
		return s.getSceneSymbol(s.gameConfig.RollCfg.Free, false, stage)
	default:
		return s.getSceneSymbol(s.gameConfig.RollCfg.Base, false, stage)
	}
}

func (s *betOrderService) getSceneSymbol(rollCfg RollCfgType, base bool, stage int8) [_colCount]SymbolRoller {
	...
	for i := 0; i < _colCount; i++ {
		...
		for j := 0; j < _rowCount; j++ {
			...
			if i > 0 && i < _colCount-1 {
				...
				if stage == _spinTypeBase && gold < s.gameConfig.GoldBaseProb && sm < _treasure {
					sm += _goldSymbol
				} else if stage != _spinTypeBase && gold < s.gameConfig.GoldFreeProb && sm < _treasure {
					sm += _goldSymbol
				}
			}
	...
	for k := needNewSymbol - 1; k >= 0; k-- {
		...
		if i > 0 && i < _colCount-1 {
			...
			if stage == _spinTypeBase {
				if gold < gameConfig.GoldBaseProb && newSymbol < _treasure {
					newSymbol += _goldSymbol
				}
			} else {
				if gold < gameConfig.GoldFreeProb && newSymbol < _treasure {
					newSymbol += _goldSymbol
				}
			}
		}
