...
func TestRtpBenchmark(t *testing.T) {
	...
	for baseRounds < _benchmarkRounds {
		...
		if svc.scene.Stage == _spinTypeFree {
			svc.scene.IsFreeRound = true
		}
		if svc.scene.Stage == _spinTypeBase {
			svc.scene.IsFreeRound = false
		}
		...
		for {
			...
			// 更新 isFree 状态（在 baseSpin 之前，用于设置状态）
			isFree = svc.client.ClientOfFreeGame.GetFreeNum() > 0
			if svc.scene.Stage == _spinTypeFree || svc.scene.Stage == _spinTypeFreeEli {
				isFree = true
				svc.scene.IsFreeRound = true
			} else {
				svc.scene.IsFreeRound = false
			...
			currentIsFree := svc.scene.IsFreeRound
			...
			if currentIsFree {
				...
				if svc.scene.RoundMultiplier > 0 && svc.scene.RoundOver {
					baseHasWin = true
				}
			}
			...
			if result.winInfo.Next && !result.SpinOver {
				// 更新场景状态（必须在 baseSpin 之前更新，因为 isFreeRound() 依赖 scene.Stage）
