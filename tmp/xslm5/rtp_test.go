...
func TestRtp(t *testing.T) {
	...
	var baseTotalWin, baseFemaleSymbolWin, baseFemaleWildWin float64
	var baseMaxCascadeSteps int

	// 免费模式统计
	var freeRounds, freeWinRounds, freeCascadeSteps, freeFullElimination int64
	var freeTreasureInFree, freeExtraFreeRounds, freeMaxFreeStreak int64
	var freeTotalWin, freeFemaleSymbolWin, freeFemaleWildWin float64
	var freeMaxCascadeSteps int
	var freeFemaleStateCount [10]int64
	var femaleKeyWins [10]float64
	...
	for baseRounds < testRounds {
		...
		var roundHasFullElimination, roundIsFree bool
		...
		for {
			...
			if isFirst {
				...
				roundWin, roundHasFullElimination = 0, false
				svc.client.IsRoundOver = false
			}

			stepStartFemaleCounts := svc.scene.FemaleCountsForFree
			roundStartFemaleCountsForFree := svc.scene.RoundFemaleCountsForFree
			...
			if isFree {
				...
				roundHasFullElimination = roundHasFullElimination || (svc.enableFullElimination && svc.hasFemaleWildWin)
			}
			...
			if debugFileOpen > 0 && fileBuf != nil && (freeModeLogOnly == 0 || isFree) {
				...
				writeSpinDetail(fileBuf, svc, gameNum, cascadeCount, isFree, triggerRound,
					stepStartFemaleCounts, roundStartFemaleCounts, stepWin, roundWin,
					roundStartFemaleCountsForFree, svc.nextFemaleCountsForFree, isFirst)
			}
	...
	printFinalStats(buf, baseRounds, baseTotalWin, baseWinRounds, baseFemaleSymbolWin, baseFemaleWildWin, baseCascadeSteps, baseMaxCascadeSteps, baseFreeTriggered,
		freeRounds, freeTotalWin, freeWinRounds, freeFemaleSymbolWin, freeFemaleWildWin, freeCascadeSteps, freeMaxCascadeSteps, freeFullElimination, freeTreasureInFree, freeExtraFreeRounds, freeMaxFreeStreak, freeFemaleStateCount, femaleKeyWins, totalBet, start)
	result := buf.String()
	fmt.Print(result)
...
func writeSpinDetail(
	buf *strings.Builder,
	svc *betOrderService,
	gameNum,
	step int,
	isFree bool,
	triggeringBaseRound int,
	stepStartFemaleCounts [3]int64,
	roundStartFemaleCounts [3]int64,
	stepWin float64,
	roundWin float64,
	roundStartFemaleCountsForFree [3]int64,
	currentRoundFemaleCountsForFree [3]int64,
	isFirstStep bool,
) {
	if step == 1 {
...
func printGridToBuf(buf *strings.Builder, grid *int64Grid, winGrid *int64Grid) {
	...
	rGrid, rWinGrid := reverseGridRows(grid), reverseGridRows(winGrid)
	for r := int64(0); r < _rowCount; r++ {
		for c := int64(0); c < _colCount; c++ {
			symbol := rGrid[r][c]
