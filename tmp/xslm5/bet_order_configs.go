...
type gameConfigJson struct {
	...
	FreeSpinCount []int64   `json:"free_spin_count"` // 免费次数配置
	RollCfg       RollConf  `json:"roll_cfg"`        // 滚轴配置
	RealData      []Reals   `json:"real_data"`       // 滚轴数据
}

type RollConf struct {
	Base RollCfgType            `json:"base"`
	Free map[string]RollCfgType `json:"free"` // key为女性符号收集状态，如"000", "001"等
}
...
type SymbolRoller struct {
	Real        int              `json:"real"`  // 滚轴数据索引
	Start       int              `json:"start"` // 起始位置
	Fall        int              `json:"fall"`  // 掉落位置
...
func (s *betOrderService) getSceneSymbol() [_colCount]SymbolRoller {
	...
	if s.isFreeRound {
		key := s.convertFemaleCountKey()
		if freeCfg, ok := s.gameConfig.RollCfg.Free[key]; ok {
			rollCfg = freeCfg
		} else {
			panic("free roll config not found for key: " + key)
		}
	}
...
// convertFemaleCountKey 根据女性符号收集状态构建配置键
func (s *betOrderService) convertFemaleCountKey() string {
	key := ""
	for i := 0; i < 3; i++ {
