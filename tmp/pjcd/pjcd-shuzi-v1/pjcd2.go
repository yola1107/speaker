...
var LINES = [20][5]int{
	...
	{1, 7, 13, 9, 5}, {11, 7, 3, 9, 15}, {1, 2, 8, 4, 5},
	{11, 12, 8, 14, 15}, {6, 12, 13, 14, 10}, {6, 2, 3, 4, 10},
	{1, 7, 8, 9, 5}, {11, 7, 8, 9, 15}, {6, 7, 3, 9, 10},
	{6, 7, 13, 9, 10}, {6, 2, 8, 4, 10}, {6, 12, 8, 14, 10},
	{1, 7, 3, 9, 5}, {11, 7, 13, 9, 15}, {1, 2, 8, 14, 15},
	{11, 12, 8, 4, 5}, {1, 12, 3, 14, 5},
}
...
func generateReels(mode GameMode) Reels {
	...
	/*
		...
		for c := 1; c <= 3; c++ {
			...
			for i := 0; i < REEL_LENGTH; i++ {
				if reels[c][i] != SC_SYMBOL && rand.Intn(10000) < wildProb {
...
func generateMatrix(reels Reels, startPos [COLS]int) (GameMatrix, WildRoundsMatrix) {
	...
	for col := 0; col < COLS; col++ {
		...
		for row := 0; row < ROWS; row++ {
			...
			symbol := reel[pos]
			matrix[ROWS-1-row][col] = symbol

			if symbol == WILD_SYMBOL {
				wildRounds[ROWS-1-row][col] = 0
			}
		}
	}
...
func simulateSingleDrop(matrix GameMatrix, wildRounds WildRoundsMatrix, reels Reels, startPos [COLS]int, w2Positions []Position) (GameMatrix, WildRoundsMatrix, [COLS]int) {
	newMatrix := matrix
	...
	for col := 0; col < COLS; col++ {
		...
		for row := ROWS - 1; row >= 0; row-- {
			if removePositions[row][col] {
				...
				newMatrix[row+emptySpots][col] = newMatrix[row][col]
				newWildRounds[row+emptySpots][col] = newWildRounds[row][col]
				newMatrix[row][col] = 0
				newWildRounds[row][col] = 0
			}
		}

		// 补格取符：从「可见窗口下一位」开始取，与 pjcd/dev-picd2 一致（Fall 推进 emptySpots 次）
		// 下一轮窗口起始 = startPos + emptySpots，使下次取符为 reel[startPos+emptySpots+ROWS]
		for row := 0; row < emptySpots; row++ {
			readPos := (startPos[col] + ROWS + row) % REEL_LENGTH
			symbol := getSymbolFromReel(reels[col], readPos)
			newMatrix[row][col] = symbol
			if symbol == WILD_SYMBOL {
				newWildRounds[row][col] = 0
			}
		}
		newStartPos[col] = (startPos[col] + emptySpots) % REEL_LENGTH
	}

	return newMatrix, newWildRounds, newStartPos
}
...
func simulateDrop(matrix GameMatrix, wildRounds WildRoundsMatrix, reels Reels, startPos [COLS]int, mode GameMode, printDetails bool, stats *GameStats, freeModeFourthRoundBonus *int) (GameMatrix, WildRoundsMatrix, [COLS]int, float64, int) {
	...
	for {
		...
		// 执行单轮掉落
		currentMatrix, currentWildRounds, currentStartPos = simulateSingleDrop(
			currentMatrix, currentWildRounds, reels, currentStartPos, w2Positions)
	...
	return currentMatrix, currentWildRounds, currentStartPos, totalPayout, rounds
}
...
func simulateGame() {
	...
	for stats.baseSpins < TOTAL_SPINS {
		...
		if scatterCount >= FREE_GAME_SCATTER_MIN {
			...
			for inFreeMode && freeSpinsRemaining > 0 {
				...
				// 免费模式中根据夺宝符号数量增加剩余次数的新逻辑
				if freeScatterCount == 2 {
					// 2个夺宝符号，增加 FREE_GAME_TWO_SCATTER_ADD_TIMES 次
					additionalGames := FREE_GAME_TWO_SCATTER_ADD_TIMES
					freeSpinsRemaining += additionalGames
					stats.freeGamesExtended++

					if printFreeDetails {
						fmt.Printf("  → 2个夺宝符号，增加免费次数: +%d次 (当前剩余%d次)\n", additionalGames, freeSpinsRemaining)
					}
				} else if freeScatterCount >= 3 {
					// 3个及以上夺宝符号，增加等同于基础模式触发免费模式时的免费spin数
					additionalGames := calculateFreeGames(freeScatterCount)
					freeSpinsRemaining += additionalGames
					stats.freeGamesExtended++
			...
			// 免费模式结束：回到基础模式仍使用之前保存的基础轮轴（与 pjcd/策划一致，不在此处重新生成）
			if inFreeMode {
				//baseReels = generateReels(BaseMode)
				//baseReelSpinCounter = 0
				currentReels = baseReels
	...
	// 与 pjcd 同量纲对比：pjcd 压测为 20/局，此处按 20/局 换算总投注与总奖金便于直接对照
	const pjcdBetPerSpin = 20.0
	if stats.baseSpins > 0 {
		compareBet := float64(stats.baseSpins) * pjcdBetPerSpin
		scale := compareBet / totalBet
		comparePayout := totalPayout * scale
		fmt.Printf("\n[与 pjcd 同量纲对比] 总投注(20/局): %.2f 总奖金: %.2f 整体RTP: %.2f%%\n",
			compareBet, comparePayout, (comparePayout/compareBet)*100)
	}
