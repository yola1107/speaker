...
// initFirstStepForSpin 初始化首轮 Spin
...
// initStepForNextStep 初始化后续 Step
...
func (s *betOrderService) updateGameOrder() error {
	gameOrder := game.GameOrder{
		...
		BonusMultiple: s.roundMultiplier,
...
func (s *betOrderService) fillInGameOrderDetails() error {
	...
	s.gameOrder.BonusDetail = s.winGridToString(s.winGrid)
	s.gameOrder.WinDetails = s.getWinDetailProto()
	return nil
}

// getWinDetailProto 返回 proto 格式的 WinDetails JSON
