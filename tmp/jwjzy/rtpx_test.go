...
const (
	...
	// 关闭 debug 文件写入，避免 100 局统计被 IO 拖慢
	debugFileOpen           = 0
	freeModeLogOnly         = 0
	maxCascadeStepsPerRound = 200
)
...
func TestRtp2(t *testing.T) {
	...
	for baseRounds < testRounds {
		...
		for {
			...
			// Round 结束处理
			if !svc.isRoundOver && cascadeCount > maxCascadeStepsPerRound {
				// 防止判奖/消除在某些盘面下无法收敛导致测试长时间运行
				svc.isRoundOver = true
				if svc.isFreeRound {
					svc.scene.FreeNum = 0
				} else {
					svc.scene.FreeNum = 0
