...
type betOrderService struct {
	...
	butterflyMultiplier int64                // 蝴蝶百搭贡献的倍数 = ButterflyCount * WildAddFourthMultiple
	gameMultiples       []int64              // 当前连击的倍数列表
...
func (s *betOrderService) buildWinInfo() *pb.Pjcd_WinInfo {
	...
	return &pb.Pjcd_WinInfo{
		...
		FreeGameMultipleIndex: proto.Int64(s.gameMultipleIndex),
		RoundWin:              proto.Float64(s.calcRoundWin()),
		//WildGrid:              s.wildStateGridToPbBoard(s.scene.WildStateGrid),
		ButterflyCount: proto.Int64(s.scene.ButterflyCount),
	}
}

// wildStateGridToPbBoard 将 WildStateGrid 转为 common.Board
func (s *betOrderService) wildStateGridToPbBoard(grid int64Grid) *pb.Board {
