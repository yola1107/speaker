...
func TestRtp(t *testing.T) {
	...
	for baseRounds < _benchmarkRounds {
		...
		// 判断当前是否为Re-spin模式
		isFreeRound := svc.scene.Stage == _spinTypeFree
		if isFreeRound {
			freeWin += stepWin
		...
		// 统计Re-spin触发（当前是Base，下次是Free）
		if !isFreeRound && svc.scene.NextStage == _spinTypeFree {
			freeTriggerCount++
			freeTime++
		}

		// 回合结束，重置服务（只在Base模式结束时）
		if svc.scene.NextStage == _spinTypeBase && !isFreeRound {
			resetTestService(svc)
...
func resetTestService(s *betOrderService) {
	s.stepMultiplier = 0
	s.lineMultiplier = 0
