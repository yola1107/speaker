...
type betOrderService struct {
	...
	isSpinOver      bool                 // 当前 spin 是否结束
	scatterCount    int64                // 夺宝符个数
	addFreeTime     int64                // 增加的免费次数
	lineMultiplier  int64                // 线倍数累计
	stepMultiplier  int64                // 本 step 总倍数（线×游戏倍）
	gameMultiple    int64                // 累计第2–4列倍数
	curGameMultiple [_colCount]int64     // 当前各列倍数
