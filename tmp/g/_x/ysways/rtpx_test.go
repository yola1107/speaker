...
const (
	...
	maxAverageSpinsPerBaseRound = 500
...
func validateRtpConfig(t *testing.T, cfg *gameConfigJson) {
	t.Helper()
	if cfg == nil {
		t.Fatal("rtpx config is nil")
	}
	for _, symbol := range _checkList {
		if symbol <= _blank || symbol >= _wild {
			t.Fatalf("_checkList contains non-paying symbol: %d", symbol)
		}
		if int(symbol) > len(cfg.PayTable) {
			t.Fatalf("_checkList symbol %d has no pay table row: pay_table rows=%d", symbol, len(cfg.PayTable))
		}
	}
	for realIdx, real := range cfg.RealData {
		for colIdx, reel := range real {
			for symbolIdx, symbol := range reel {
				if symbol <= _blank || symbol > _treasure {
					t.Fatalf("real_data contains undefined symbol: real=%d col=%d index=%d symbol=%d valid=1..%d",
						realIdx, colIdx, symbolIdx, symbol, _treasure)
				}
			}
		}
	}
