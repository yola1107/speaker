...
type betOrderService struct {
	...
	gameMultiple      int64                // 连续消除倍数（含蝴蝶贡献）
	gameMultipleIndex int64                // 轮次倍数表索引
	lineMultiplier    int64                // 线倍数合计
	stepMultiplier    int64                // Step 倍数
	winInfos          []WinInfo            // 中奖信息
	nextSymbolGrid    int64Grid            // 下一把 step 符号网格
	symbolGrid        int64Grid            // 符号网格（3行5列）
	winGrid           int64Grid            // 中奖网格（3行5列，只包含参与中奖的行）
...
func gameOrderToResponse(gameOrder *game.GameOrder) (*pb.Pjcd_BetOrderResponse, error) {
	...
	return &pb.Pjcd_BetOrderResponse{
		...
		FreeTimes:         &gameOrder.FreeTimes,
		NewFreeTimes:      &winDetail.NewFreeTimes,
		SymGrid:           int64GridToArray(symbolGrid),
		WinGrid:           int64GridToArray(winGrid),
		MulIndex:          &winDetail.MulIndex,
		BaseMultipliers:   winDetail.BaseMultipliers,
		FreeMultipliers:   winDetail.FreeMultipliers,
		WildEliCount:      &winDetail.WildEliCount,
		TotalWildEliCount: &winDetail.TotalWildEliCount,
		WildEliMultiple:   &winDetail.WildEliMultiple,
