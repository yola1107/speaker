...
func (s *betOrderService) buildBoardFromReelData(reelData [][]int64, reelMode int) [_colCount]SymbolRoller {
	...
	for c := 0; c < _colCount; c++ {
		...
		for r := 0; r < _rowCount; r++ {
			symbol := reel[(start+r)%reelLen]
			roller.BoardSymbol[_rowCount-1-r] = symbol
		}
		symbols[c] = roller
	}
	return symbols
...
func (s *betOrderService) generateFullReelData(isFree bool) [][]int64 {
	...
	for c := 0; c < _colCount; c++ {
		reelData[c] = s.generateReelSymbolsFull(c, isFree)
	}
	return reelData
}

func (s *betOrderService) generateReelSymbolsFull(c int, isFree bool) []int64 {
