...
type gameConfigJson struct {
	...
	ScatterCount      []int64   `json:"scatter_count"`       // 触发免费所需最少 Scatter 数量（未配置则回退默认）
	FreeSpins         []int64   `json:"free_spins"`          // 单次触发赠送的免费次数（未配置则回退默认）
	LvCollectCount    []int64   `json:"lv_collect_count"`    // 樱花收集外观阈值
	ChangeSymbolId    []int64   `json:"change_symbol_id"`    // 百变樱花最终揭示为哪种符号
	BaseOpenWeights   []int     `json:"base_open_weight"`    // BaseGame 对应权重
	FreeOpenWeights   []int     `json:"free_open_weight"`    // FreeGame 对应权重
	SakuraTriggerRate int64     `json:"sakura_trigger_rate"` // 樱吹雪触发概率（万分比）
...
func (s *betOrderService) validateMysteryConfigs() {
	...
	if len(s.gameConfig.SakuraReels) != len(s.gameConfig.SakuraReelsWeight) {
		panic("invalid sakura reels weight")
	}
	...
	if len(s.gameConfig.ScatterCount) != len(s.gameConfig.FreeSpins) {
		panic("scatter_min and free_times length not match")
	}
}
