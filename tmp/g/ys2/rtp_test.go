...
func autoSelectRtpBonus(t *testing.T, svc *betOrderService) {
	t.Helper()
	freeNum, err := svc.selectFreeBonus(_autoFreeType)
	if err != nil {
		t.Fatalf("rtp auto select bonus failed: scatter=%d err=%v", svc.scene.ScatterNum, err)
	}
	svc.addFreeTime = freeNum
}
