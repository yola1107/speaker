...
const (
	...
	// maxAverageSpinsPerBaseRound：每个基础局允许的平均 baseSpin 调用上限。
	// testRounds 很大时不能使用固定总上限，否则正常跑数也会在中途误报为“免费不结束”。
	maxAverageSpinsPerBaseRound = 100
...
func TestRtp2(t *testing.T) {
	...
	svc.initGameConfigs()
	//validateRtpConfig(t, svc.gameConfig)
	...
	for baseRounds < testRounds {
		...
		for {
			...
			if err := svc.ensureBonusSelected(); err != nil {
				autoSelectRtpBonus(t, svc)
				continue
			}
			...
			if totalSpins > maxTotalSpins {
				t.Fatalf("TestRtp2: 全局 spin 超过 %d（平均每基础局上限=%d）。baseRounds=%d/%d freeRounds=%d FreeNum=%d FreeTimes=%d isFreeRound=%v stage=%d steps=%d isRoundOver=%v nWin=%d",
					int64(maxTotalSpins), int64(maxAverageSpinsPerBaseRound), baseRounds, int64(testRounds), freeRounds,
					svc.scene.FreeNum, svc.scene.FreeTimes, svc.isFreeRound, svc.scene.Stage, svc.scene.Steps,
					svc.isRoundOver, len(svc.winInfos))
			}
			...
			if svc.isRoundOver {
				...
				if isFree {
					...
					// 基础模式回合结束时，如果触发了免费游戏，RTP 固定自动选第2档。
					if svc.scene.BonusState == _bonusStatePending {
						autoSelectRtpBonus(t, svc)
					}
					if svc.scene.BonusState == _bonusStateSelected && svc.scene.FreeNum > 0 {
						baseFreeTriggered++
					}
					// 记录触发免费游戏的基础局数
					if svc.isFreeRound {
				...
				if svc.scene.FreeNum <= 0 {
					resetRtpBetCounters(svc)
					freeRoundIdx = 0
					if baseRounds%interval == 0 {
						totalWin := baseTotalWin + freeTotalWin
						printBenchmarkProgress(buf, baseRounds, totalBet, baseTotalWin, freeTotalWin, totalWin, baseWinRounds, freeWinRounds, freeRounds, baseFreeTriggered, start)
...
func writeStepSummary(buf *strings.Builder, svc *betOrderService, step int, isFree bool, stepWin, roundWin float64) {
	...
	if len(svc.winInfos) == 0 {
		...
		if svc.isRoundOver {
			if svc.scene.BonusState == _bonusStatePending {
				fprintf(buf, "\t💎  等待选档: BonusState=%d, s=%d\n", _bonusStatePending, svc.scatterCount)
			}
			if isFree && svc.addFreeTime > 0 {
				...
				fprintf(buf, "\t本手触发免费: 夺宝=%d, +免费次数=%d, 剩余免费=%d, NextStage=%d，stepmul=%d, enterfree 💎💎💎\n",
					svc.scatterCount, svc.addFreeTime, svc.scene.FreeNum, svc.scene.NextStage, svc.stepMultiplier)
			}
		}
		return
	}
	...
	for _, elem := range svc.winInfos {
		fprintf(buf, "\t符号:%2d, 路数:%2d, 命中: %d, 赔率: %4.2f, 奖金: %4.2f\n",
			elem.Symbol, elem.LineCount, elem.SymbolCount, float64(elem.Odds), float64(elem.Multiplier))
	}
	...
	fprintf(buf, "\tMode=%d, Stage=%d, Steps=%d, stepMul=%d, lineMul=%d, 本回合累计 step 倍数: %.2f\n",
		isFreeMode, svc.scene.Stage, svc.scene.Steps, svc.stepMultiplier, svc.lineMultiplier, roundWin)
	if !svc.isRoundOver {
