...
func writeReelInfo(buf *strings.Builder, svc *betOrderService) {
	...
	fprintf(buf, "\n")
	var downCount [_colCount]int
	for c := 0; c < _colCount; c++ {
		downCount[c] = len(svc.scene.DownLongs[c])
	}
	fprintf(buf, "MysCount：%v \n ", downCount)

	//fprintf(buf, "DownLongs每列块数：")
	//for c := 0; c < _colCount; c++ {
	//	fprintf(buf, "%d:%d ", c, len(svc.scene.DownLongs[c]))
	//}
	//fprintf(buf, "\n")
}
...
func writeStepSummary(buf *strings.Builder, svc *betOrderService, step int, isFree bool, stepWin, roundWin float64) {
	...
	for _, elem := range svc.winInfos {
		...
		fprintf(buf, "\t符号: %2d, 连线: %d, 路数：%d, 赔率: %d, sysMul: %d, 奖金:%2d\n",
	...
	fprintf(buf, "\tMode=%d, Stage=%d, Steps=%d, RoundMul=%d, stepMul=%d, lineMul=%d, sysMul=%d, limit=%v, 本回合累计 step 倍数: %.2f\n",
		isFreeMode, svc.scene.Stage, svc.scene.Steps, svc.scene.RoundMultiplier, svc.stepMultiplier, svc.lineMultiplier, svc.mysMul, btoi(svc.limit), roundWin)
	if !svc.isRoundOver {
