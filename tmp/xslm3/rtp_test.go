		...
		for {
			...
			if debugFileOpen > 0 && fileBuf != nil {
				...
				if freeModeLogOnly > 0 && !isFree {
					...
					writeSpinDetail(fileBuf, svc, roundDisplayIdx, cascadeCount, isFree, triggerRound, stepStartFemaleCounts, roundStartFemaleCounts, stepWin, roundWin)
				}
			}
...
func writeSpinDetail(buf *strings.Builder, svc *betOrderService, gameNum, step int, isFree bool, triggeringBaseRound int, stepStartFemaleCounts [3]int64, roundStartFemaleCounts [3]int64, stepWin float64, roundWin float64) {
	if step == 1 {
		writeRoundHeader(buf, svc, gameNum, isFree, triggeringBaseRound, stepStartFemaleCounts, getTreasureCount(svc.spin.symbolGrid))
	}
...
func writeRoundHeader(buf *strings.Builder, svc *betOrderService, gameNum int, isFree bool, triggeringBaseRound int, femaleStart [3]int64, initialTreasure int64) {
	if isFree {
...
func writeStepSummary(buf *strings.Builder, svc *betOrderService, step int, isFree bool, stepStartFemaleCounts [3]int64, roundStartFemaleCounts [3]int64, stepWin float64, roundWin float64) {
	...
	roundDelta := [3]int64{
		final[0] - roundStartFemaleCounts[0],
		final[1] - roundStartFemaleCounts[1],
		final[2] - roundStartFemaleCounts[2],
	}
