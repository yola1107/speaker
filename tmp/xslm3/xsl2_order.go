...
func (s *betOrderService) betOrder(req *request.BetOrderReq) (map[string]any, error) {
	...
	// 判断是否免费回合（首次step时根据免费次数判断，后续step在initStepForNextStep中判断）
	if s.isFirst {
		s.isFreeRound = s.client.ClientOfFreeGame.GetFreeNum() > 0
	}
...
func (s *betOrderService) buildResultMap() map[string]any {
	ret := map[string]any{
		...
		"symbolGrid":              convertGridForDisplay(s.spin.symbolGrid),
		"winGrid":                 convertGridForDisplay(s.spin.winGrid),
		"winResults":              convertWinResultsForDisplay(s.spin.winResults),
...
func convertGridForDisplay(grid *int64Grid) [][]int64 {
	if grid == nil {
		return nil
	}
	rowCount := int(_rowCount)
	colCount := int(_colCount)
	result := make([][]int64, rowCount)
	for r := 0; r < rowCount; r++ {
		srcRow := rowCount - 1 - r
		row := make([]int64, colCount)
...
func convertWinResultsForDisplay(results []*winResult) []map[string]any {
	if len(results) == 0 {
		return nil
	}
	display := make([]map[string]any, 0, len(results))
	for _, wr := range results {
		display = append(display, map[string]any{
			...
			"winGrid":            convertGridForDisplay(&wr.WinGrid),
		})
	}
	return display
}
